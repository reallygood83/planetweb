...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'observation_sessions' AND policyname = 'Users can manage own observation sessions'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'daily_observations' AND policyname = 'Users can manage own daily observations'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'observation_analysis' AND policyname = 'Users can manage own observation analysis'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'observation_templates' AND policyname = 'Users can manage own templates'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'teacher_evaluations' AND policyname = 'Users can manage own teacher evaluations'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'student_achievements' AND policyname = 'Users can manage own student achievements'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'achievement_templates' AND policyname = 'Users can manage own achievement templates'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'evaluation_record_mappings' AND policyname = 'Users can manage own evaluation mappings'
  ) THEN
...
-- ===========================================
-- 4️⃣ 인덱스 생성 (성능 최적화) - IF NOT EXISTS 추가
-- ===========================================

-- 관찰 기록 시스템 인덱스
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_observation_sessions_user_class') THEN
    CREATE INDEX idx_observation_sessions_user_class ON observation_sessions(user_id, class_id);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_observation_sessions_date') THEN
    CREATE INDEX idx_observation_sessions_date ON observation_sessions(session_date);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_daily_observations_user_student') THEN
    CREATE INDEX idx_daily_observations_user_student ON daily_observations(user_id, student_name);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_daily_observations_date') THEN
    CREATE INDEX idx_daily_observations_date ON daily_observations(observation_date);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_daily_observations_keyword') THEN
    CREATE INDEX idx_daily_observations_keyword ON daily_observations(category_id, keyword_id);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_observation_analysis_user_student') THEN
    CREATE INDEX idx_observation_analysis_user_student ON observation_analysis(user_id, student_name);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_observation_analysis_period') THEN
...
-- 교사 평가 시스템 인덱스
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_teacher_evaluations_user_plan') THEN
    CREATE INDEX idx_teacher_evaluations_user_plan ON teacher_evaluations(user_id, evaluation_plan_id);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_teacher_evaluations_class') THEN
    CREATE INDEX idx_teacher_evaluations_class ON teacher_evaluations(class_id);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_teacher_evaluations_date') THEN
    CREATE INDEX idx_teacher_evaluations_date ON teacher_evaluations(evaluation_date);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_student_achievements_evaluation') THEN
    CREATE INDEX idx_student_achievements_evaluation ON student_achievements(teacher_evaluation_id);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_student_achievements_student') THEN
    CREATE INDEX idx_student_achievements_student ON student_achievements(user_id, student_name);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_student_achievements_level') THEN
...
-- ===========================================
-- 6️⃣ 기본 데이터 삽입 - 개별 INSERT로 수정
-- ===========================================

-- 기본 관찰 카테고리 데이터 삽입 (첫 번째)
