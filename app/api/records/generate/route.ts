...
[교과학습발달상황 작성 지침 - 중요!]
1. **형식 요구사항**:
   - 절대 영역별로 구분하지 마세요 (듣기·말하기, 읽기, 쓰기 등으로 나누지 않음)
   - 하나의 연결된 문단으로 작성하세요
   - 볼드체나 제목 표시 금지 (**, ##, - 등 사용 금지)

2. **내용 구성**:
   - 학습 활동과 성취 수준을 자연스럽게 연결하여 서술
   - 평가 결과에 따른 긍정적 표현 사용:
     * 매우잘함/잘함: 구체적 성취와 능력 강조
     * 보통: 현재 수준과 발전 가능성 균형있게 표현
     * 노력요함: "~하려는 노력을 보임", "점차 향상되고 있음" 등

3. **표현 원칙**:
   - 부정적 표현 금지: "못함", "미흡함" → "노력 중", "관심을 가지기 시작함"
   - 구체적 학습 활동과 교사 관찰 내용을 자연스럽게 연결
   - 학생의 자기평가 응답에서 긍정적 태도 반영
...
위 정보를 종합하여 하나의 연결된 문단으로 ${recordType}을 작성해주세요. 
영역별 구분 없이 자연스럽게 이어지는 서술로 학생의 학습 성장을 표현하세요.
`
}
